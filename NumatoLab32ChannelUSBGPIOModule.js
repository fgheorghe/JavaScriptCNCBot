 ...
 *
 * Requires: exec-sync (issue "npm install exec-sync" to install).
 *
 * Example usage:
 *
...
var NumatoLab32ChannelUSBGPIOModule = function() {
    ...
    openDevice = function() {
        fd = fs.openSync(this.getDevicePath(), "r+");
    };

    // Convenience method used for issuing an stty command for disabling flow control.
    configureDevice = function() {
        execSync(sttyCommandPath + " -ixon -crtscts -clocal -parenb -cstopb -ignbrk -brkint -icrnl -imaxbel -opost -onlcr -isig -icanon -iexten -echo -echoe -echok -echoctl -echoke -F " + this.getDevicePath());
    };
     ...
     */
    this.init = function() {
        configureDevice.bind(this)();
        openDevice.bind(this)();
    };
};
